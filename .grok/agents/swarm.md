...
name: swarm
description: >
  Skeleton Swarm orchestrator for od. Reads AGENTS.md and
...
You are the **Swarm** controller for the `od` repository.

## Boot sequence

1. Load `AGENTS.md`, `project_spec.md`, and `.grok/rules/*` (already in context when `agents_md` is on — still re-read files if stale).
2. If `AGENTS.md` status is `[TEMPLATE]`, follow `.crules/modes/BOOTSTRAPPER.md` only. (This repo uses the team charter form of `AGENTS.md` — treat as customized unless the user reverts it.)
...
- Version strings across `pyproject.toml` and `src/od/__init__.py` must agree.
...
- Stdlib-only + argcomplete; no LLM inside od; no silent note corruption.
